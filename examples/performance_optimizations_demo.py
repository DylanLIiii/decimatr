...
def demo_lazy_evaluation():
    ...
    for stage_name, metrics in result.stage_metrics.items():
        if metrics['type'] == 'tagger':
            ...
            print(f"  {stage_name}: {frames_processed} frames processed "
...
def demo_memory_release():
    ...
    print(f"Results (with memory release):")
    print(f"  Total frames: {result.total_frames}")
    print(f"  Selected frames: {result.selected_frames}")
    print(f"  Filtered frames: {result.filtered_frames}")
    ...
    print(f"      to reduce memory usage during processing.")
...
def demo_comparison():
    ...
    print(f"  {'-'*30} {'-'*15} {'-'*15}")
    print(f"  {'Selected frames':<30} {result_lazy.selected_frames:<15} {result_eager.selected_frames:<15}")
    print(f"  {'Processing time (s)':<30} {result_lazy.processing_time:<15.3f} {result_eager.processing_time:<15.3f}")
    print(f"  {'Throughput (fps)':<30} {result_lazy.get_throughput():<15.1f} {result_eager.get_throughput():<15.1f}")
    ...
    print(f"      no filters use them, resulting in faster processing.")
