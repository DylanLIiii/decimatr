...
class ActorPipeline:
    ...
    def __init__(
        self,
        pipeline: List[Union[Tagger, Filter]],
        n_workers: int = 4,
        use_gpu: bool = False,
        ...
        try:
            ...
            for i, component in enumerate(self.pipeline):
                ...
                if isinstance(component, Tagger):
                    ...
                    raise TypeError(
                        f"Unknown component type at stage {i}: {type(component)}"
                    )
            
            self._initialized = True
            logger.info(
                f"ActorPipeline initialized successfully with "
                f"{len(self.actor_pools)} stages"
            )
    ...
    async def process_frame(
        ...
        if not self._initialized:
            raise RuntimeError(
                "ActorPipeline not initialized. Call initialize() first."
            )
        
        if not isinstance(packet, VideoFramePacket):
            raise TypeError(
                f"packet must be a VideoFramePacket, got {type(packet)}"
            )
    ...
    async def _cleanup_actors(self) -> None:
        ...
        for stage_name, actor_ref in self.actor_pools.items():
            try:
                ...
                logger.warning(
                    f"Error destroying actor for {stage_name}: {e}"
                )
